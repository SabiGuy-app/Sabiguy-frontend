...
export default function StartNavigation() {
  ...
  // Start Navigation button below.
  //   "pending" -> <WaitingForPaymentModal /> shows the countdown, button disabled
  //   "paid"    -> both modals unmount, button enabled
  //   "expired" -> <PaymentExpiredModal /> shows instead, button stays disabled
  // ---------------------------------------------------------------
  ...
  useEffect(() => {
    if (!bookingId || paymentStatus !== "pending") return;
    ...
    const poll = async () => {
      try {
        ...
        // ⚠️ confirm the real field/value with your backend — placeholder guess
  ...
  const handlePaymentModalClose = () => {
    // Just hide the popup — stay on this page, keep polling for payment
    // in the background. The modal reappears on its own if it expires,
    // since that check is separate from `waitingModalDismissed`.
  ...
  // Same fields AlertsCard.jsx already reads off `alert` for pickup,
  // dropoff, price breakdown and delivery date — reused here so the
  // modal shows the exact same numbers the provider saw before accepting.
  ...
  const modalBookingPrice =
    latestBooking?.bookingPrice ?? alert?.BookingPrice ?? 0;
  const modalPlatformFee =
    latestBooking?.platformFee ?? alert?.platformFee ?? 0;
  const modalRiderReceives =
    latestBooking?.riderReceives ?? alert?.RiderReceives ?? 0;
  ...
  return (
    <ProviderDashboardLayout>
      <div className="py-4">
        ...
        {/* Pops up on load (paymentStatus starts "pending") and unmounts
          once paymentStatus flips to "paid"/"expired" — or once the
          provider dismisses it early with the X icon. */}
