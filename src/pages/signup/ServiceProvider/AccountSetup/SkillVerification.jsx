...
setLoading(true);
    ...
    setErrorMessage("");
     ...
     service: services.map((s) => ({
      ...
      const response = await axios.post (`${import.meta.env.VITE_BASE_URL}/provider/job-service`, payload, {
          headers: {
  ...
  const handleEditService = (service, index) => {
   ...
   const handleSaveService = (newService) => {
    if(editingService) {
      setServices((prev) => 
      prev.map((service, index) => (index === editingService.index ? newService : service))
    );
...
<div className="flex flex-col gap-3 mt-3">
     ...
     <button type="button" className="flex items-center gap-1 border border-gray-300 rounded-md w-40 py-1 font-semibold text-gray-700 bg-white hover:bg-[#8BC53FBF] transition"
