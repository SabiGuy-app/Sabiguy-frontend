...
import {  FiChevronLeft, FiStar, FiCalendar, FiMapPin, FiPlus } from "react-icons/fi";
import { Wallet, CreditCard } from "lucide-react";
import Success from "../../pages/Dashboard/sections/Bookings/PaymentSucessfull"

export default function BookingSummaryScreen({ bookingData, onBack, onConfirm }) {
          ...
          <div className="flex items-center gap-4 p-4 rounded-lg">
            <div className="w-16 h-16 rounded-full overflow-hidden bg-gray-200 flex-shrink-0">
              {bookingData?.provider?.avatar ? (
                <img src={bookingData.provider.avatar} alt={bookingData.provider.name} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full bg-gradient-to-br from-blue-400 to-blue-600" />
              )}
            </div>
            <div className="flex-1">
              <h3 className="font-semibold text-gray-900 mb-1">{bookingData?.service}</h3>
                            <h3 className=" text-gray-300 text-sm mb-1">{bookingData?.service}</h3>

              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-gray-600">{bookingData?.provider?.name}</span>
<div className="flex items-center gap-1">
                                <FiStar className="text-yellow-400 fill-yellow-400" size={12} />
                                
                                  <span className="text-sm text-gray-400">{bookingData?.rating}</span>
...
{bookingData?.status || "Pending Confirmation"}               </span>
          </div>
        </div>
 <div className="mb-6">
          <h2 className="text-sm font-semibold text-gray-900 mb-4">Schedule Details</h2>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-semibold text-gray-900">₦{bookingData?.serviceCost?.toLocaleString()}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">Service Charge</span>
              <span className="font-semibold text-gray-900">₦{bookingData?.serviceCharge?.toLocaleString()}</span>
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-gray-200">
              <span className="text-sm font-semibold text-gray-900">Total Amount</span>
              <span className="text-lg font-bold text-green-600">₦{bookingData?.totalAmount?.toLocaleString()}</span>
            </div>
          </div>
        </div>
        <div className="mb-6">
            ...
            <label className="flex items-center gap-3 p-2 border-2 rounded-lg cursor-pointer transition-all hover:bg-gray-50">
              ...
              <div className="flex items-center gap-3 flex-1">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">₦{bookingData?.walletBalance?.toLocaleString()}</p>
                </div>
              </div>
            </label>
             <label className="flex items-center gap-3 p-2 border-2 rounded-lg cursor-pointer transition-all hover:bg-gray-50">
